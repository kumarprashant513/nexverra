...
// Add a check for placeholder MONGO_URI from Atlas
if (MONGO_URI.includes('cluster.mongodb.net')) {
    console.error('\n❌ Configuration Error: Your MONGO_URI environment variable seems to be using a placeholder hostname.');
    console.error('Please replace "cluster.mongodb.net" with the actual connection string from your MongoDB Atlas dashboard.');
    console.error('It should look like: mongodb+srv://<username>:<password>@my-cluster-name.xxxxx.mongodb.net/myFirstDatabase?retryWrites=true&w=majority\n');
    process.exit(1);
}
